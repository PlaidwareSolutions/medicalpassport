...
## Session 13 — draft Hindi/Telugu/Urdu marketing locales (2026-08-13)

The homepage (S1–S14) now has **candidate** translations for **Hindi, Telugu, Urdu** — status **DRAFT / PROFESSIONAL REVIEW REQUIRED**, never published. English remains the canonical source.

- **Routing (static export, no runtime):** the parked `app/_locales/[locale]` is un-parked to `app/(locales)/[locale]` (route group). `buildLocales()` gates emission — production emits only `PUBLISHED_LOCALES` (`en`); staging (`MARKETING_ENV=staging`) also emits `/hi/ /te/ /ur/` for review. Draft routes are page-level noindexed + under the staging host noindex.
- **Dictionaries:** `lib/dictionaries/{hi,te,ur}.ts` (full `Record<MessageKey,string>`; `clinics.*`/`lead.*` stay English — `/for-clinics/` is English-only V1). Brand names Latin; demo values/autonyms identical across locales; claim strength matched to English.
- **Locale switcher:** `components/LocaleSwitcher.tsx` — native `<details>` disclosure, keyboard-accessible, no flags, RTL-aware, static links; shown by `SiteHeader` when the build emits >1 locale.
- **RTL:** Urdu renders `<html dir="rtl">`; embedded Latin (brand/QR/numbers) via the Unicode bidi algorithm; **no Nastaliq webfont** (approved system/Noto stack). Verified visually on staging.
- **Review banner:** `components/ReviewBanner.tsx` — slim staging-only strip on draft locales; never on English/production.
- **SEO:** hreflang/canonical/sitemap/OG-`alternateLocale` all keyed on `PUBLISHED_LOCALES`, so drafts are never advertised. OG `locale` per page. Legal links on non-English pages point to the **English** `/privacy/` `/terms/` marked "(English)" (§51).
- **Performance (§44):** `SiteHeader` is now a **server** component (build-time `t()`), with a tiny client `HeaderStickyCta` carrying no i18n; `LeadForm` sources English directly. Result: **no dictionary ships to the client on any route** — homepage First Load JS is 104 kB (was 110), `/hi /te /ur` = 104 kB, `/for-clinics/` dropped 123→111 kB.
- **Checker:** `scripts/check-locales.mjs` (wired into `deploy:staging`) validates key parity / empties / duplicates / placeholders and reports draft coverage; fails only if a **published** locale is incomplete.

Verified live on staging: `/hi/ /te/ /ur/` 200 + noindexed; correct `lang`/`dir` (ur = rtl); 0px overflow at 320/390/1280; switcher opens with 4 language links; review banner on drafts only; English regression clean. Reviewer artifacts: [marketing-translation-review.md](marketing-translation-review.md), [localization-glossary.md](localization-glossary.md).
