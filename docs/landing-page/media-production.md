...
## Addendum — brand film published (2026-08-30, owner-directed)

Owner supplied a Runway-generated 9:16 commercial (53s, 1440×2560, WITH soundtrack — the first non-silent clip) and directed its placement on the home page. Reviewed frame-by-frame before publish (hospital-worry scene → pill drawer → photographing a prescription → relief → closing card "MedicinePassport.App — Know what you're taking. Available now."): shows only shipped behavior, no painted product UI. Encoded with the §pipeline's video settings plus audio (h264 crf22 + AAC 128k / vp9 crf34 + Opus 96k, 720×1280; 8.4MB/5.1MB from an 85MB source; poster from the 2.2s brand card). Published as `r8-commercial` via publish-media.mjs (hash-named, immutable, verified). **Site behavior differs from the silent loops by necessity:** `CommercialFilm` is strictly click-to-play (poster + play button; `<video controls>` mounts only on the user's gesture — audio never autoplays, and the reduced-motion zero-autoplaying-video invariant is preserved). Section `Film` renders between Reveal and Know only when the asset is published. Source moved to gitignored `artifacts/marketing-media/sources/`. Note: the film's spoken/text content is owner-supplied creative and has not been through the MKT claims ledger row-by-row; the on-page copy around it introduces no new claims. No captions VTT — the soundtrack's dialogue (if any) is unverified; a real caption file is future owner work, the accessible label carries the visual description.

**Behavior revision (2026-08-31, owner-directed):** the film now AUTOPLAYS when its section scrolls ≥55% into view, overriding the site's never-autoplay-audio rule for this one asset at the owner's explicit direction. Implemented as an honest ladder (browsers refuse unmuted playback before any page gesture — scrolling is not one): (1) try unmuted; (2) refused → play muted with a prominent "Tap for sound" pill (one tap unmutes); (3) reduced-motion/Save-Data → no autoplay, original poster + click-to-play. Scroll-away pauses, scroll-back resumes, and a pause made via the native controls is respected (no forced resume). Both ladder branches verified in Chromium (permissive: unmuted plays + pauses on scroll-away; simulated strict policy: muted plays + unmute tap works).
