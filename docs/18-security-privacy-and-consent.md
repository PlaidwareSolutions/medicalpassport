...
### Identity & sessions
OTP sign-in (argon2id-hashed OTPs, expiry, attempt/resend limits, enumeration-safe, provider webhook verification, abuse monitoring); opaque session tokens (hashed at rest), rotation on refresh, device-centric listing + revocation.

**Trusted-device login (ADR-14, [24](24-open-decisions-and-assumptions.md)):** the "known device" half of this section's original "new device → step-up OTP" intent, built the other direction first since it's the half that directly removes real friction (an OTP costs a real SMS or voice call). A device that completes OTP with "Remember this device" checked (default on) is issued a separate, long-lived, hashed, individually-revocable credential (`UserDevice.trustTokenHash`, mirroring the session-token pattern exactly — never a stateless/self-verifying token, per ADR-5) via an httpOnly cookie distinct from the session/refresh cookies. `POST /auth/device-login` accepts just a phone number and, if the trust cookie matches a non-revoked device belonging to that same phone, issues a session with no OTP at all — every failure mode (missing cookie, revoked, wrong phone) returns the identical generic error so this can never become an enumeration oracle. Trust has **no time-based expiry** — it is renewed on every use (capped at ~400 days by browser cookie-lifetime policy, effectively indefinite for anyone who opens the app at least once a year) and ends only via explicit Sign out or a remote device revoke from the Devices & Sessions screen, both of which revoke the trust credential *and* any live session on that device in the same action. A shared device where a different family member completes their own OTP supersedes (revokes) whoever's trust was there before. The "unknown/suspicious device → step-up OTP" half of the original intent remains unbuilt — every device without valid trust already gets full OTP today, so nothing here weakens that baseline, but true velocity/anomaly-based step-up is future work, not yet needed since there is no lower-friction path than OTP to step up *from* for a new device.

Admin accounts: email + password + TOTP MFA mandatory + optional Cloudflare Access in front of `admin-web`.
