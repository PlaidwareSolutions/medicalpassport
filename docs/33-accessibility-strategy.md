...
- **Text-to-speech on every clinical content surface** (medication explanations, warnings, instructions) with per-locale voice detection and server-audio fallback ([32](32-browser-capability-and-fallback-matrix.md)). **Built (self-guiding UX chunks 1–2)**: `lib/read-aloud.ts` + `ReadAloud` — every signed-in screen's header speaks its purpose (pre-generated per-locale audio, `guide.screen.*`), the medicine-detail header additionally reads the medicine's own summary, and every `ClinicalContentBlock` reads its full approved text via a locale-matched browser voice; the button renders nothing when neither audio nor a suitable voice exists, and `e2e/guidance.spec.ts` pins presence/absence per route × locale in CI. Pre-generated MP3s pending the first `generate:audio` run (OD-16).
