...
**Document-first, with a closed-vocabulary value layer.** Originally per-analyte values were deferred outright; they now exist in `report_values` — but deliberately *not* as an open-domain clinical store. The analyte vocabulary is a curated ~30-entry constant in `@medpass/domain` (`REPORT_ANALYTES`, canonical unit each, provisional pending clinical validation per docs/34); anything outside it uses an explicitly labelled `other`. The uploaded document remains the record of truth; the values are transcription, not extraction.

### ★ report_values
One structured value transcribed off a test report (docs/07 screen 44, docs/10 H-25). `report_id FK`, `patient_profile_id FK` (denormalized per the global convention), `analyte text` (closed vocabulary, TEXT not enum — renamable without migration surgery while provisional), `other_label NULL` (required iff `other`), **`entered_value text` — immutable, exactly as typed off the paper** (global rule: original vs normalized always separate), `numeric_value numeric(10,3) NULL` — the server-parsed twin, present only when the text parses cleanly ("Negative", "<5.7", "13.2-17.0" stay text-only and never trend); **nothing ever renders the parsed number**, every surface shows the entered text verbatim, so a parse bug cannot change what a doctor sees. `reference_text NULL` — the report's own printed range, display-only: the app never compares a value against it and never flags high/low (docs/02: not a diagnostic system). `recorded_by_user_id`, `deleted_at`. No update path — a typo is delete + re-add (glucose precedent). Indexes: `(patient_profile_id, analyte)` for per-analyte history, `(report_id)`. Report soft-delete never cascades; reads filter through the parent. Sync: no — online-only, matching the parent.
...
Structured dosing per medication (typed, from confirmed input). `patient_medication_id FK`, `dose_quantity numeric`, `dose_unit`, `frequency_code` enum `OD|BD|TDS|QID|SOS|HS|pattern|alternate_day|weekly|fortnightly|monthly|custom` (`fortnightly`/`monthly` added Stage 4 follow-up — see below), `pattern text NULL` (e.g. `1-0-1`), `food_instruction` enum `before|with|after|any|bedtime`, `timing_slots jsonb`, `duration_days int NULL`, `original_text NULL` (immutable), `confirmed_by`, `confirmed_at`, `dose_unit_confirmed_at NULL`, `row_version`. Sync: yes.

**`dose_unit_confirmed_at` is not the same as `confirmed_at`**, which is stamped on every instruction regardless. It records whether a human actually *chose* the dose unit. Until 2026-07-25 the add/edit screens hardcoded `"tablet"` on every save (and the scan-review screen kept doing so until this was fixed), so historic rows carry a unit nobody picked. That was harmless while the unit only ever rendered as text; it stopped being harmless when the medicines list began drawing it as a picture, which a low-literacy patient trusts over the text beside it. Null drives the confirm-your-medicine-type prompt (docs/07 screen 9). Backfilled as confirmed only where it can be known — any non-tablet unit (the default was `tablet`, so anything else was picked by a person) or anything written on or after the fix. **The invariant to keep: a creation path that defaults the unit must leave this null.**
...
### scheduled_doses
Materialized due doses (rolling window generated by cron). `medication_schedule_id FK`, `due_at timestamptz`, `slot_label`, `status` enum `upcoming|due|taken|skipped|missed|snoozed|could_not_take|unavailable|problem|taken_other_time|cancelled`, unique `(schedule_id, due_at)`. Retention: 24 months then aggregated — **the 24-month deletion is now enforced** by the `retention-cleanup` cron (rows due >24 mo ago are deleted; `dose_events`/`notifications` FKs are `ON DELETE SET NULL`, so the durable record detaches rather than disappearing; aggregation not built — `dose_events` carry enough to aggregate later). Sync: yes (window).
