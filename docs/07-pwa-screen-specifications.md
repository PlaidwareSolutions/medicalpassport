...
- **Built (self-guiding UX chunk 4):** `InstallEducationCard` on Home, gated on ≥1 medicine ∧ install actually available (captured `beforeinstallprompt`, or iOS Safari manual steps) ∧ not previously dismissed (`medpass_a2hs_dismissed`, forever); glyph + benefit line + listen button; `beforeinstallprompt` captured at module scope in `lib/install-prompt.ts` because Chrome can fire it before hydration and never refires. e2e (`education.spec.ts`) proves render + forever-dismissal with a synthesized prompt event. Analytics events remain TODO comments (no client emitter exists). Also offered on the public welcome screen (screen 1) with no dismiss button — the page is transient, and tapping past it must not suppress the Home card later.
...
- **Help part built (self-guiding UX chunk 6):** `/help` — public, standalone, no auth — with the docs/01 20-question FAQ in four themed groups (answers permanently expanded with their listen buttons — a disclosure widget is itself a hurdle for the docs/01 target user; navigation-only wording per docs/02), the tour replay, the install topic (reachable even after a Home dismissal), and the "call 112 / your doctor" boundary card. Entry points: Profile card; Welcome instead offers the public tour as its secondary action ("Take a tour"). Not built, deliberately: search (20 grouped questions don't need it yet), the contact/support path (no support channel is defined anywhere — see docs/24), the patient-maintained emergency card and its quick-access/consent mechanics (a PHI feature, its own stage), and `help_topic_viewed` analytics (no client emitter).
...
- **List:** grouped by doctor (screen 45's shared records make "everything Dr. X prescribed" the natural reading), each group collapsed to one disclosure row — doctor's name and how many prescriptions — expanding on tap. Groups ordered by their most recent visit, prescriptions newest-first within each; records with no doctor gather in one group at the end. Each expanded row: visit date and how many files/medicines it holds (counts exclude deleted upload stubs and deleted medicines). A record with no date recorded falls back to when it was filed rather than sinking to the bottom forever.
...
- **Detail (`/prescriptions/[id]`):** attached files (view via short-lived presigned link; an incomplete upload shows as unavailable rather than offering a broken link) **plus take-photo/choose-file controls to append more pages any time after filing** — a prescription is routinely several pages, and the record must be able to grow; the gallery picker is multi-select. Also the medicines linked to it, "Add a medicine from this prescription" (opens the normal add flow with the prescription pre-linked), and "Link a medicine you already added."
...
- **Detail (`/reports/[id]`):** the recorded details plus attached files (view via short-lived presigned link; an incomplete upload shows as unavailable rather than offering a broken link), **plus take-photo/choose-file controls to append more pages any time after filing** — a multi-page report filed with one photo must be able to grow; the gallery picker is multi-select.
...
- **Structured values (added after the initial pass):** the report detail screen records individual values transcribed off the paper — analyte from a closed, grouped ~30-entry vocabulary (`REPORT_ANALYTES`, canonical unit each, English-only on purpose: the patient is matching a report printed in English), entered value kept verbatim and immutable, optional reference range copied off the report. Add and delete only, one at a time; no edit (a typo is delete + re-add). The app **never** computes high/low, colours a value, or renders its parsed numeric twin — transcription, not interpretation (docs/02, docs/10 H-25). Values appear in the doctor-visit summary inside the existing report rows, verbatim, with no value ids on the public path.
- **Per-analyte history (`/reports/values?analyte=`):** one value across every report, newest first (test date, falling back to filing date), each row linking to its source report. A plain list — no charts this pass. Report values only; check-up entries (screen 42) are never merged in, and the screen says so.
- **Out of scope (this pass):** trend charts; OCR of report values (screen 42's never-fabricated promise applies).
