...
- **List:** grouped by doctor (screen 45's shared records make "everything Dr. X prescribed" the natural reading), groups ordered by their most recent visit and prescriptions newest-first within each; records with no doctor gather in one group at the end. Each row: visit date and how many files/medicines it holds. A record with no date recorded falls back to when it was filed rather than sinking to the bottom forever.
