...
import { fetchClinicalContent } from "@medpass/clinical-content";
...
/**
 * Fetches a candidate for one (ingredient, kind) pair from openFDA/DailyMed
 * and stores it as a draft awaiting human review (docs/13, docs/34 Gate
 * 6/OD-6) — never patient-visible until an admin with `content_approve`
 * approves it. A `null` adapter result (no reliable data found) is a
 * normal, successful outcome: no row is created, and the job still
 * succeeds — this must not be treated as a failure. A thrown adapter error
 * (network/5xx/429) propagates up to the caller's normal failJob/retry/
 * dead-letter handling, exactly like every other processor.
 */
