...
import { ReportValuesSection } from "./ReportValuesSection";

/**
 * The V1 report detail (docs/07 screen 44), kept whole as the fallback for a
 * report id the diagnostics API does not know — a pre-Phase-4 record reached
 * from the timeline or an old link. Unchanged behaviour: details, values,
 * filed documents.
 */
export function LegacyReportDetail({ id }: { id: string }) {
  const { t } = useI18n();
  const timezone = useActiveTimezone();
  const router = useRouter();

  const { report, error, reload } = useReport(id);
