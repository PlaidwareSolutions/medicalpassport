...
import {
  ...
  adminProductMetricsQuerySchema,
...
// V2 Phase 10 (docs_v2/06 P10): treatment journey.
...
export const ROUTES: RouteDoc[] = [
  ...
  { method: "GET", path: "/v1/admin/rules/quality", summary: "Gate 3 alert-quality dashboard", description: "Per rule version over `from..to` (default trailing 30 days): findings raised, acknowledged, reviewed with a professional, dismissed as not relevant, acknowledgement rate, false-positive rate (dismissed ÷ raised), median seconds to acknowledgement. Counts only — no finding, profile or clinical value.", tags: [T.adminRules], auth: "admin", duty: "view_rules", query: adminRulesQualityQuerySchema, ...hw(S.anyObject) },
  ...
  { method: "GET", path: "/v1/admin/metrics/product", summary: "Product metrics: counts per event per day", description: "PHI-free aggregate over `product_events` (docs_v2/06 P1-7, docs_v2/14 §6). Days are cut on Asia/Kolkata; `from` defaults to seven days before `to`, `to` to now, at most 92 days per call. Never an id, never a value.", tags: [T.adminOperations], auth: "admin", duty: "view_operations", query: adminProductMetricsQuerySchema, ...hw(S.anyObject) },
  ...
  // keep working unchanged until their V2.5 sunset (docs_v2/05 §15).
  {
    ...
    description:
      "Returns one presigned upload per requested page, in page order. A `kind` sent here is the patient's own choice and outranks the classifier permanently (docs_v2/09 §4). An `Idempotency-Key` replays the same document and authorizations — the offline capture replay resumes on it (docs_v2/05 §14).",
    tags: [T.documentsV2],
    auth: "patient",
    rateLimit: "document_upload",
  ...
  // ───────────────────────── V2 Phase 10: treatment journey (docs_v2/06 P10) ─────────────────────────
  { method: "GET", path: "/v1/profiles/current/clinical-relationships", summary: "Clinical relationship edges (condition ↔ medicine ↔ result ↔ measurement ↔ provider)", description: "Reading the list also refreshes the derived edges. Every derived edge is `origin: inferred`, `status: suggested` and carries the `basis` it was read off (a medicine's `reasonConditionId`, a prescription's practitioner, or an analyte the condition is usually followed with — the last only when the patient already has results for it). A suggestion is a question, never a claim; nothing here asserts that one thing caused another (docs_v2/10 §1, exit gate M10). Answered edges survive every later refresh.", tags: [T.clinicalProfile], auth: "patient", headers: [PROFILE], scope: "view_profile", query: clinicalRelationshipsQuerySchema, ...hw(S.itemsOf(S.entity)) },
  { method: "POST", path: "/v1/clinical-relationships/:id/confirm", summary: "Confirm a suggested link", description: "Records who answered yes (`confirmedByUserId`, `confirmedAt`) and moves `verification` to `patient_confirmed`. The row keeps `provenanceSource: system_derived` and its `basis`, so how the edge was found stays visible. Another profile's edge is a 404.", tags: [T.clinicalProfile], auth: "patient", headers: [PROFILE, CLIENT], scope: "edit_profile", ...hw(S.entity) },
  { method: "POST", path: "/v1/clinical-relationships/:id/dismiss", summary: "Dismiss a suggested link", description: "Records who answered no and stops the edge being suggested again.", tags: [T.clinicalProfile], auth: "patient", headers: [PROFILE, CLIENT], scope: "edit_profile", ...hw(S.entity) },
  { method: "GET", path: "/v1/profiles/current/conditions/:id/journey", summary: "Condition hub: medicines, tracked tests and measurements, doctors, suggestions and before/after views", description: "Four independently scoped sections (docs_v2/04 §2.2): a caregiver without `view_tests` gets the condition and an empty `results` with `sections.tests: false`, not a 403 for the whole screen. `medicines[].linkSource` says whether the medicine itself names this condition as its reason or the patient confirmed a suggested link. `beforeAfter` carries at most three views and no comparison of any kind.", tags: [T.clinicalProfile], auth: "patient", headers: [PROFILE], scope: "view_profile", ...hw(S.anyObject) },
  { method: "GET", path: "/v1/medications/:id/before-after", summary: "Readings around a medicine's start date, and at roughly 30 and 90 days after", description: "docs_v2/06 P10-2. Three fixed, server-owned windows — baseline (−30…0 days), day 30 (+15…+45) and day 90 (+75…+105) — each returned with its absolute bounds and the descriptive statistics the trend endpoints already produce (count, average, min, max). There is deliberately no difference, percentage, direction, rating or verdict field: two numbers either side of a date are a coincidence in time, and computing anything from them would be a clinical claim this app is not entitled to make (docs_v2/10 §1). `hasEnoughPoints: false` means too few of the patient's own readings, never \"no effect\". Requires `view_medications` plus `view_tests` (with `analyteKey`) or `view_measurements` (with `concept`).", tags: [T.medications], auth: "patient", headers: [PROFILE], scope: "view_medications", query: beforeAfterQuerySchema, ...hw(S.anyObject) },
];
