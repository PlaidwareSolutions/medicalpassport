...
export async function writeAudit(tx: Tx, entry: AuditEntry): Promise<void> {
  if (isFullClient(tx)) {
    await tx.$transaction(async (inner) => {
      ...
      await appendAuditRow(inner, entry, await readChainTail(inner));
    });
  } else {
    await tx.$executeRaw(CHAIN_LOCK_SQL);
    await appendAuditRow(tx, entry, await readChainTail(tx));
  }
}

/**
 * Appends several entries under ONE acquisition of the chain lock, in the
 * order given — the deferred queue's writer (`deferred.ts`, ticket 0.18).
 * Each row chains to the one appended just before it, so a batch of N costs
 * one lock wait instead of N.
 */
export async function writeAuditBatch(client: PrismaClient, entries: readonly AuditEntry[]): Promise<void> {
  if (entries.length === 0) return;
  await client.$transaction(async (tx) => {
    await tx.$executeRaw(CHAIN_LOCK_SQL);
    let prevHash = await readChainTail(tx);
    for (const entry of entries) prevHash = await appendAuditRow(tx, entry, prevHash);
  });
}

/** The current tail's hash — only meaningful while the chain lock is held. */
async function readChainTail(tx: Prisma.TransactionClient): Promise<string | null> {
  const prev = await tx.auditEvent.findFirst({
    orderBy: { seq: "desc" },
    select: { rowHash: true },
  });
  return prev?.rowHash ?? null;
}

/** Inserts one row linked to `prevHash` and returns the new row's hash. */
async function appendAuditRow(tx: Prisma.TransactionClient, entry: AuditEntry, prevHash: string | null): Promise<string> {
